...
<Info>
  **When to use it:** consumer-facing products where users shouldn't think about gas. Solvers and aggregators running their own submission pipelines usually prefer `walletBroadcast` - see the [quickstart](/rfq-api/quickstart).
</Info>
...
| Order endpoint   | Prepared tx comes from preflight             | POST the signed order to `/api/v1/rfq/orders`    |
| Firmness         | Firm - signed escrow commitment              | Firm - identical guarantee, no last look         |
| Gas              | Paid by taker                                | Abstracted from the user                         |
| Funding          | Direct from wallet                           | Escrow lock: Permit2, EIP-3009, or resource lock |

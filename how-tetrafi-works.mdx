...
A single routing, settlement, and compliance engine underlies everything, built around [regulated stablecoins and tokenized real-world assets at institutional size](/why-tetrafi). Spreads compress through solver competition, with depth aggregated from OTC desks, CEXs, DEXs, and regulated LPs. Aggregation is the familiar half; where it settles is not. Every fill lands in **immutable contracts on-chain**, atomically and across chains, rather than on a venue's internal ledger - so there is no balance pre-funded to a counterparty, no principal exposed mid-trade, and the trade composes with the on-chain assets and venues it touches. On top of it, TetraFi sells you one of two distinct trading contracts, depending on what your execution needs:

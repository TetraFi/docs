...
Whether flow goes through the [RFQ API](/rfq-api/introduction) or the [Router API](/router-api/introduction), the workspace fee applies identically and is disclosed the same way - revenue starts with your first settled trade. The [implementation guide](/core-concepts/monetization) has the field-level detail, and a note on how fees will work on [Multilateral Netting](/netting) and the [Shared Collateral Network](/collateral) when they ship: the netting fee follows the saving, never the gross, and your partner fee stays on the instant quote.

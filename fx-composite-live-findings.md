...
**Registrar gap (caught pre-run, fixed 2026-07-06):** the Arc deploy ran without `FX_PLAN_REGISTRAR`/`RELAY_PLAN_REGISTRAR` in env, so the deployed FxContinuationModule had NO allowed registrar — corridor-2's `registerArrivalPlan` would have reverted `NotRegistrar`. Fixed on-chain via owner `setRegistrar(0x90c6…16B4, true)` (tx `0x8d53d110…`, matches the RFQ relayer `WITHDRAWAL_FORWARDING_RELAYER_PRIVATE_KEY` address); `FX_PLAN_REGISTRAR` added to the monorepo `.env` so redeploys self-wire. **Mainnet checklist addition (step 3): set `FX_PLAN_REGISTRAR` to the PROD relayer EOA before the Arc deploy, and post-deploy assert `isRegistrar(relayer) == true` on the module.**

## 10. UI fix — funder-permit chain switch (2026-07-06, live-reported)

Symptom (user screenshot, order parked "Arrived on destination"): clicking **Sign funding permit** threw `An internal error was received. Details: Provided chainId "5042002" must match the active chainId "84532" (viem@2.38.3)`. Root cause: the funder permit is an **Arc** EIP-712 signature (`domain.chainId = 5042002`) but the wallet was connected to Base Sepolia (84532); the signer refuses a domain/active-chain mismatch. This affects EVERY composite whose FX junction differs from the wallet's current chain — i.e. always, since the bridge leg leaves the user on origin/destination, not Arc.

Fix (`tetrafi-ui`, commit `aa97ae24`): `ContinueRouteBanner` now switches the wallet to the permit's `domain.chainId` before requesting the signature — Arc is registered in `walletChains` so wagmi can add/select it — and the button reads **"Switch to Arc Testnet & sign"** when a switch is pending; a rejected switch surfaces a clear inline error instead of the raw signer mismatch. Chain decision extracted to framework-free helpers `continuationSignChainId` / `needsChainSwitch` (8 unit tests, per the repo's no-interactive-RTL convention for this component). Prod-correctness follow-up (`77c141ce`): the wallet-chains comment's stale Arc-mainnet id `1243` corrected to `5042` (registry-realigned) with a note that `arcMainnet(5042)` must join `mainnetChains` when the Arc-mainnet venue launches, else this same switch step has no chain to select on prod.

## 11. UI — StableFX-aware composite status page (2026-07-06, live-reported)

Symptom (user, corridor-1 order in the UI): the status page treated the Circle StableFX continuation leg as a generic single **"Solver fill"** stage — no forward visibility of the remaining FX steps, no StableFX tx on the status/activity page, and no detailed breakdown of the continuation leg. Overview showed the source burn + the FX settlement (FILL TRANSACTION) but not the CCTP mint on the destination chain.

Root cause (`tetrafi-ui/src/features/trade-flow/model/order-status/route-status.ts`): `productBridgeKind` had no `arcStableFx` case, so the FX leg fell through to `'solver'` → `legStageLabels` returned a single `'Solver fill'`. The FX leg's accept→fund→settle sub-flow and its `circleStatus`/`settlementTransactionHash` (both already on the wire — settlement.data passes through verbatim) were never surfaced.

Fix (`tetrafi-ui` commit `2419a9bd`):
- New `'fx'` bridge kind; the FX leg contributes three user-visible stages — **FX trade accepted → Fund FX trade → FX settling on Circle** — so the whole plan (incl. the not-yet-done FX steps) renders up front as *pending* in the pipeline stepper (the forward visibility the user asked for).
- `fxLegSubProgress` advances the FX sub-stage on the FX-specific `circleStatus` (`confirmed`→funding, `taker_funded`→settling, `complete`→settled), because the coarse `routeStatus` collapses the FX leg into `arrived→executing→settled`.

...
* **One unit of collateral, many uses at once.** What restaking proved for security, applied to settlement and credit: the same committed dollar stands behind redemptions, settlement fronting, venue credit, and cross-chain fills simultaneously - encumbered only while a draw is live, and only to that one obligation. A buffer is one client, one use; shared collateral is as many uses as the network has products.
...
Four products, one capital base, one integration shape - a credit framework, not a product line. Every new use case joins as an adapter over the same vaults, bringing its demand to the standing pool - which is also how [Multilateral Netting](/netting) plugs in: vault liquidity backstops residuals and prices early exits from a netting position. And the four are the wedge, not the boundary: the same collateral base is designed to back guarantees, credit facilities, insurance cover, and tokenized strategies as the network opens.

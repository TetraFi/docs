...
* **Applications join as adapters.** A guarantee product, a credit facility, a receivables desk - each ships as one adapter over the standing pool: no new pool to bootstrap, no liquidity to rent, and depositors who have already underwritten the framework.
* **Insurance is a flagship use case.** Underwriting capacity is shared collateral in its oldest form - reinsurance has priced one pool of capital against many risks for centuries. An on-chain cover line joins as one more adapter: capacity drawn from the standing pool, and capital that keeps working between claims.

...
description: "One pool of committed capital replacing every idle buffer in institutional settlement - coming soon."
---
...
The Shared Collateral Network retires those buffers. **Capital is committed once, into permissioned [programmable vaults](/collateral/vaults) with identifiable counterparties, and every settlement gap draws on the same pool through an adapter - never through another buffer.** What makes that safe is enforceable commitments: capital moves only against a verified obligation, with the proof of how it comes back checked in the very transaction that draws it. The layer that builds it is programmable collateral; its founding product is programmable credit - financing the gap between when value is promised and when it settles.

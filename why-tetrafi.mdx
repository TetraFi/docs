...
**Today** - **settlement and execution rails.** The first product suite delivers composable cross-chain liquidity aggregation, settlement, and execution with configurable policy enforcement.

**Next** - **the [Shared Collateral Network](/collateral).** Programmable collateral, committed once and cross-collateralised across the whole settlement stack - T+0 redemption, instant settlement, venue prefunding, solver credit - one deep, fungible pool retiring the dead capital institutions strand in single-purpose buffers today. Capital efficiency as infrastructure.

The bigger play - **programmable collateral.** The four products are only the wedge - the product is the framework itself. On-chain lending has its standard; **shared, programmable collateral has none**: the same capital backing many obligations at once - restaking's insight, reinsurance's economics - still built one-off per use case, keeping capital shallow and expensive. Standardised as adapters over one collateral network - guarantees, receivables, insurance, tokenized strategies - every use case inherits the network's underwriting, deepens the same pool, and gives capital one more reason to stay.

**Alongside** - **[Multilateral Netting](/netting).** It offsets the network's obligations against each other, so only the residual ever pays a settlement toll.

...
The netting engine is the same story. It is **domain-blind** - it clears who owes whom without caring what created the debt - so one engine nets corporate receivables, real-world payments, the books inside other settlement networks, and loan books on lending markets, on-chain and off. Each of those rebuilds set-off from scratch today, or does without it. As a **composable primitive** it plugs into rails that already exist rather than replacing them: TetraFi's are its first book, not its only one, and every book that joins deepens the same graph.

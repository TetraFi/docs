...
#### Longer term - Programmable collateral and netting primitives
...
The netting engine is built the same way - a **standalone primitive**, not internal plumbing. Wherever obligations run in both directions, the gross flow is optional: receivables between corporates, obligations inside other settlement networks, loan books on lending markets. TetraFi's own rails are simply its first user. The engine plugs onto a stack that already carries the obligations, so any network can add **multilateral set-off** to what it settles today, and build its own products on top.

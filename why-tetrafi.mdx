...
The live product suite: composable cross-chain liquidity aggregation, settlement, and execution with configurable policy enforcement - the rail everything below settles through. See [How TetraFi Works](/how-tetrafi-works).

### Next: The Shared Collateral Network

**Programmable collateral**, committed once and cross-collateralised across the whole settlement stack - [T+0 redemption](/collateral/instant-redemption), [instant settlement](/collateral/instant-settlement), [venue prefunding](/collateral/venue-prefunding), [solver credit](/collateral/solver-credit) - one deep, fungible pool retiring the dead capital institutions strand in single-purpose buffers today. [Capital efficiency as infrastructure](/collateral).

### The Bigger Play: Programmable Credit

The four products are only the wedge. The bigger play is the layer itself: **programmable credit** as the new primitive - one standard framework for guarantees, receivables, insurance, and tokenized strategies, replacing the one-off builds each runs on today - infrastructure anyone can build on, and any allocator can underwrite.

### Alongside: Multilateral Netting

[Multilateral Netting](/netting) offsets the network's obligations against each other, so only the residual ever pays a settlement toll.

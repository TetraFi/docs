...
The netting engine is the same. It only needs to know who owes whom and by when, so **one engine** clears invoices, payment flows, loan repayments, and the obligations inside other settlement networks - all of which still pay in full today for amounts that cancel out. What funds a run is just as modular: an on-chain vault, a balance on an exchange, a bank transfer - whatever a participant already holds. TetraFi's rails are just the first to use it; anyone can add it to what they already settle.

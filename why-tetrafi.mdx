...
The netting engine is the same. Clearing has always needed an institution in the middle carrying everyone's risk, so it exists only inside a few closed, supervised markets. Settling atomically, solving the graph, and keeping each book private make that institution unnecessary. What is left only needs to know who owes whom and by when: it integrates into someone else's stack, funds a run from whatever they already hold - an on-chain balance, a balance on an exchange, a bank transfer - and nets whatever book they point it at, from the loans on one lending market to the receivables inside one payment network. TetraFi's rails are simply its first user.

...
The netting engine is the same. It only needs to know who owes whom and by when, so **one engine** clears invoices, payment flows, lending and borrowing, energy imbalances, and the books inside other settlement networks - all of which still pay in full today for amounts that cancel out. What funds a run is just as modular: an on-chain vault, a balance on an exchange, a bank transfer. Every new use case joins as an adapter, inherits the engine, and nets against everything already on the graph. TetraFi's rails are simply the first to build on it.

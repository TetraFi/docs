...
The netting engine is the same. It only needs to know who owes whom and by when, and what funds a run is just as modular - an on-chain vault, a balance on an exchange, a bank transfer. So it drops on top of someone else's stack and nets whatever book they point it at: the loans on one lending market, the receivables inside one payment network, a desk's trade finance. Their counterparties, their rules, **their own graph** - joining ours is an option, not a requirement. Either way, obligations that cancel stop being settled in full. TetraFi's rails are simply its first user.

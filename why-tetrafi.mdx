...
The netting engine is the same. Clearing has always meant someone standing between counterparties, so it lives inside a few closed, supervised markets. On-chain consensus, atomic settlement, and trusted execution under zero-knowledge proofs do that work instead: **no central counterparty**, no margin posted to one, no mutualised default fund. What is left integrates into someone else's stack and nets whatever book it is pointed at - the loans on one lending market, the receivables inside one payment network - funded from an on-chain balance, an exchange, or a bank transfer. TetraFi's rails are simply its first user.

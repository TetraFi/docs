...
The netting engine is the same. It only needs to know who owes whom and by when, and what funds a run is just as modular - an on-chain vault, a balance on an exchange, a bank transfer, whatever a participant already holds. So others **build on it** rather than buy it: standardised as adapters - trade finance, receivables, lending and borrowing, energy imbalances, equities settlement - every new book inherits the engine and nets against everything already on the graph, instead of paying in full today for amounts that cancel out. TetraFi's rails are simply the first to use it.

...
The bigger play - **programmable credit.** The four products are only the wedge - the product is the framework itself. On-chain borrowing and lending has its standard; shared, programmable collateral - the same capital securing many obligations at once - is a new primitive with none. Today every such use case is a one-off build that allocators must research from scratch, so capital stays shallow and expensive. Standardised as adapters over shared collateral - guarantees, receivables, insurance, tokenized strategies - every new use case inherits the network's underwriting, deepens the same pool, and gives capital one more reason to stay.

...
The netting engine is the same story. Set-off itself is old, but every network that wants it rebuilds the whole engine - the shared graph, the solver, the validity proof, the backstop ladder - so netting stays the privilege of whoever can afford to build it. As a **composable primitive** it is integrated instead: any book of two-way obligations clears through it - receivables between corporates, positions inside other settlement networks, loan books on lending markets. TetraFi's own rails are its first user, not its only one - anyone can add set-off to what they already settle, build their own products on top, and every book that joins deepens the same graph.

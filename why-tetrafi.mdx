...
The netting engine is the same. Clearing has always meant someone standing between counterparties, so it lives inside a few closed, supervised markets. On-chain consensus, atomic settlement, and trusted execution under zero-knowledge proofs do that work instead: **no central counterparty**, no margin posted to one, no mutualised default fund. What is left is **a primitive**, not a venue: it integrates into any settlement stack, nets whatever book it is pointed at - one lending market's loan book, one payment network's receivables - and funds itself from an on-chain balance, an exchange, or a bank transfer. TetraFi's rails are simply the first thing built on it.
